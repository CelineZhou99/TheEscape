...
void DoorStateLocked::OnCollideWithPlayer(World& world)
{
	...
	Item* item = nullptr;
	if (item = world.GetPlayer()->GetInventory()->FindInInventory(required_key_type))
	{
		world.SetDialogue(_dialogue_locked_right_key.get());
		App::PlaySound(DOOR_OPEN_SOUND);
		world.GetPlayer()->GetInventory()->RemoveFromInventory(item);
		_door->SetState(DoorStateType::UNLOCKED);
		world.GetCurrGoal()->AddToUnlockedDoors(_door->GetDoorId());
	}
	else if (item == nullptr)
	{
		ItemType alt_key_type = required_key_type == ItemType::KEY ? ItemType::KEY_ESCAPE : ItemType::KEY;
		if (item = world.GetPlayer()->GetInventory()->FindInInventory(alt_key_type))
		{
			world.SetDialogue(_dialogue_locked_wrong_key.get());
		}
		else
		{
			world.SetDialogue(_dialogue_locked_no_key.get());
		}
	}
}
